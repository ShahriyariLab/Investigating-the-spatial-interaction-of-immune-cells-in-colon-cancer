...
IC_Loc_DG: Read IC data and projects it onto a DG space.
Smoothen: Applies a pre-diffusion to the IC
...
if int(prompt1)>0:
    ...
    prompt3 = input('What is the minimum number of cells allowable after remeshing? (default = 4400)')
    prompt4 = input('What is the maximum number of cells allowable after remeshing? (default = 4500)')
    prompt5 = input('What is the original size of your mesh cells. User should refer to the mesh .geo file. (default=0.012)')
    refine = int(prompt2)
...
mesh= Mesh()
xdmf = XDMFFile(mesh.mpi_comm(), "Mesh/Mesh.xdmf")
xdmf.read(mesh)
...
with XDMFFile("Mesh/boundaries.xdmf") as infile:
    infile.read(mvc2, "f")
...
#Reference Mesh
###############################################################
mesh0 = Mesh('Mesh/Mesh.xml')
Volume0 = MeshFunction('size_t' , mesh0 , 'Mesh/Mesh_physical_region.xml' )  #saves the interior info of the mesh
bnd_mesh0 = MeshFunction('size_t', mesh0 , 'Mesh/Mesh_facet_region.xml')  #saves the boundary info of the mesh
...
Th_0,Tc_0,Tr_0,Dn_0,M_0,C_0,maxTh,maxTc,maxTr,maxDn,maxM,maxC= IC_Loc_DG(mesh,SDG)
Tn0 = project(Constant(IC[Input2,0]),S1)
Th0 = project(Th_0,S1)
Tc0 = project(Tc_0,S1)
Tr0 = project(Tr_0,S1)
Dn0 = project(Dn_0,S1)
D0 = project(Constant(0),S1)
M0 = project(M_0,S1)
C0 = project(C_0,S1)
N0 = project(Constant(IC[Input2,8]),S1)
H0 = project(Constant(0),S1)
mu10 = project(Constant(0),S1)
mu20 = project(Constant(0),S1)
Igamma0 = project(Constant(0),S1)
Gbeta0 = project(Constant(0),S1)
##############################################################
...
##############################################################
Th0,Tc0,Tr0,Dn0,M0,C0 = smoothen(Th0,Tc0,Tr0,Dn0,M0,C0,S1,mesh)
##############################################################
...
###############################################################
#Finalizing initial conditions by assigning them to subspaces
###############################################################
Tn0 = interpolate(Tn0, Mixed_Space.sub(0).collapse())
Th0 = interpolate(Th0, Mixed_Space.sub(1).collapse())
Tc0 = interpolate(Tc0, Mixed_Space.sub(2).collapse())
Tr0 = interpolate(Tr0, Mixed_Space.sub(3).collapse())
Dn0 = interpolate(Dn0, Mixed_Space.sub(4).collapse())
D0 = interpolate(D0, Mixed_Space.sub(5).collapse())
M0 = interpolate(M0, Mixed_Space.sub(6).collapse())
C0 = interpolate(C0, Mixed_Space.sub(7).collapse())
N0 = interpolate(N0, Mixed_Space.sub(8).collapse())
H0 = interpolate(H0, Mixed_Space.sub(9).collapse())
mu10 = interpolate(mu10, Mixed_Space.sub(10).collapse())
mu20 = interpolate(mu20, Mixed_Space.sub(11).collapse())
Igamma0 = interpolate(Igamma0, Mixed_Space.sub(12).collapse())
Gbeta0 = interpolate(Gbeta0, Mixed_Space.sub(13).collapse())
...
for n in range(num_steps):
     ...
     if j>=1:
         ...
         + ((Th-Th_n)/k)*v2*dx+D_cell*dot(grad(Th),grad(v2))*dx+coeff*Th*div(u__)*v2*dx-((pars['lambda_{T_hD}']*D+pars['lambda_{T_hM}']*M+pars['lambda_{T_hmu_1}']*mu1)*Tn-(pars['delta_{T_hmu_2}']*mu2+pars['delta_{T_hT_r}']*Tr+pars['delta_{T_h}'])*Th)*v2*dx\
         + ((Tc-Tc_n)/k)*v3*dx+D_cell*dot(grad(Tc),grad(v3))*dx+coeff*Tc*div(u__)*v3*dx-((pars['lambda_{T_CT_h}']*Th+pars['lambda_{T_CD}']*D)*Tn-(pars['delta_{T_Cmu_2}']*mu2+pars['delta_{T_CT_r}']*Tr+pars['delta_{T_C}'])*Tc)*v3*dx\
         + ((Tr-Tr_n)/k)*v4*dx+D_cell*dot(grad(Tr),grad(v4))*dx+coeff*Tr*div(u__)*v4*dx-((pars['lambda_{T_rT_h}']*Th+pars['lambda_{T_rmu_2}']*mu2+pars['lambda_{T_rG_beta}']*Gbeta)*Tn-(pars['delta_{T_rmu_1}']*mu1+pars['delta_{T_r}'])*Tr)*v4*dx\
         + ((Dn-Dn_n)/k)*v5*dx+D_cell*dot(grad(Dn),grad(v5))*dx+coeff*Dn*div(u__)*v5*dx-(pars['A_{Dn}']-pars['alpha_{D_ND}']*(pars['lambda_{DH}']*H+pars['lambda_{DC}']*C)*Dn-(pars['delta_{DH}']*H+pars['delta_{D}'])*Dn)*v5*dx\
         + ((D-D_n)/k)*v6*dx+D_cell*dot(grad(D),grad(v6))*dx+coeff*D*div(u__)*v6*dx-((pars['lambda_{DH}']*H+pars['lambda_{DC}']*C)*Dn-(pars['delta_{DH}']*H+pars['delta_{DC}']*C+pars['delta_{D}'])*D)*v6*dx\
         + ((M-M_n)/k)*v7*dx+D_cell*dot(grad(M),grad(v7))*dx+coeff*M*div(u__)*v7*dx-((pars['lambda_{Mmu_2}']*mu2+pars['lambda_{MI_gamma}']*Igamma+pars['lambda_{MT_h}']*Th)*(pars['M_0']-M)-pars['delta_{M}']*M)*v7*dx\
